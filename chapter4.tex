...
\begin{figure}[ht] 
  {\centering
    \subfigure{\includegraphics[width=0.4\textwidth]{ch4_figs/kt.pdf}}
    \subfigure{\includegraphics[width=0.4\textwidth]{ch4_figs/ca.pdf}}\\
    \subfigure{\includegraphics[width=0.4\textwidth]{ch4_figs/sis.pdf}}
    \subfigure{\includegraphics[width=0.4\textwidth]{ch4_figs/akt.pdf}}\\
    \caption[A comparison of jet clustering algorithms]{Jets clustered with various algorithms: $k_{T}$ (upper left), Cambridge-Aachen (upper right), SIScone (lower left),
      and anti-$k_{T}$ (lower right).}
...
between the different sources of \met we use another variable called \htt, which is the negative vector sum of all selected jets and leptons in the event. While \htt has lower resolution than the \met,
it relies only on the higher \pt selected objects and not the softer objects failing the selection. To exploit the fact that \htt and \met are less correlated in events with incidental missing energy,
...
\begin{equation}
\label{eqn:metld}
 \met LD = \met\times0.00397 + \htt\times0.00265.
\end{equation}

\noindent The coefficients are tuned to scale the \met term by 60\% and the \htt term by 40\%, which were empirically found to deliver the best signal efficiency and background rejection.
...
\begin{table}[hbtp]
\centering
\caption[ELECTRON EFFECTIVE AREAS FOR PILEUP CORRECTION]{Electron effective areas for the pileup correction}
\begin{tabular}{lcc}
\hline
...
\begin{table}[hbtp]
\centering
\caption[MUON EFFECTIVE AREAS FOR PILEUP CORRECTION]{Muon effective areas for the pileup correction}
\begin{tabular}{lcc}
\hline
$|\eta|$ range & effective area (A) \\
\hline
...
\centering
\small
\caption[MUON SELECTION REQUIREMENTS]{MUON SELECTION REQUIREMENTS}
...
\label{tab:muonIDs}
    \begin{tablenotes}
      \small
    \item NOTE: Extra requirements are applied for fakeable objects that fail the lepton MVA requirement,
...
\centering
\small
\caption[ELECTRON SELECTION REQUIREMENTS]{ELECTRON SELECTION REQUIREMENTS}
...
\label{tab:eleIDs}
    \begin{tablenotes}
      \small
      \item NOTE: Requirements on each of the three electron selections. In some cases, the cut values change for different $\eta$ ranges.
