...
The control region used for measuring the fake rate is referred to as the measurement region. The measurement region is heavily enriched in QCD multijet events, which are an excellent
...
While the signal region vetos events with low mass dilepton pairs, the $t\bar{t}\gamma^{*}$ process with $\gamma^{*}\rightarrow l^{+}l^{-}$ still contributes as a background when one of the leptons fails
selection cuts or is out of the acceptance. The nominal \ttz sample is generated with $m_{l^{+}l^{-} >$ 10 GeV, and we therefor use a $t\bar{t}Z/\gamma^{*}$ sample generated at 1 $< m_{l^{+}l^{-} <$ 10 GeV
and an additional \ttbar sample which covers the $ m_{l^{+}l^{-} <$ 1 GeV phase space. The \ttbar sample is generated with Pythia, which can decay a low-virtuality $\gamma^{*}$.
